...
### Added
- HMAC nonce support for Worker requests. Signed requests now include `X-D1-Nonce` and sign `timestamp.nonce.body` to make replay detection practical.
- Per-isolate Worker nonce tracking for HMAC replay detection, with timestamp-window pruning to avoid unbounded memory growth.
- Configurable D1 transaction behavior via `transaction_mode` / `CF_D1_TRANSACTION_MODE`, including `silent`, `log`, and `exception` modes.
- `D1TransactionException` for explicit transaction-usage failures when `transaction_mode=exception`.
- `--strict` support for `d1:info`, allowing CI workflows to fail when health checks fail.
- Session reset hooks for Worker connections in long-lived runtimes.
- Worker request body size limits and JSON parsing validation for malformed payloads.

### Changed
- `DB::transaction()`, manual `beginTransaction()`, `commit()`, and `rollBack()` now consistently respect `transaction_mode`.
- `transaction_mode=log` now logs only once per connection/request to avoid noisy duplicate warnings during a single transaction lifecycle.
- `commit()` delegates to Laravel's parent commit lifecycle while keeping D1 commit behavior as a no-op.
- Query logger timing is reported in milliseconds.
- Schema dump and import upload paths now stream large SQL files instead of loading them fully into memory.
- Retry classification better detects retryable `SELECT` / `WITH` statements.
- HMAC and transaction documentation now reflects nonce signing, replay detection, and manual transaction method behavior.

### Fixed
- Malformed JSON responses from D1 query, PDO, PDOStatement, batch, and query logger paths are normalized into typed driver errors instead of leaking raw `JsonException` in supported paths.
- Worker batch endpoints validate statement shape before preparing SQL.
- Query logger records malformed JSON responses as failed query logs without throwing.
- Circuit breaker failure accounting records one failure per call even when retries are involved.
- Required PDO extensions are declared in `composer.json`.
...
[Unreleased]: https://github.com/TanDuy03/cloudflare-d1-database/compare/v0.11.0...HEAD
[0.11.0]: https://github.com/TanDuy03/cloudflare-d1-database/compare/v0.10.0...v0.11.0
[0.10.0]: https://github.com/TanDuy03/cloudflare-d1-database/compare/v0.9.1...v0.10.0
